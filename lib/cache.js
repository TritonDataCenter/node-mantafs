...
//
// The 'wb_age' configuration value is used to indicate how long a file can
// be dirty in the cache before we schedule a writeback. 'wb_age' is not a
...
//    for the file in, at most, 'wb_age' after the restart. If the server was
//    down for longer than 'wb_age', then the writeback will be scheduled
//    immediately.
...
//    writes back each file that has been on the list for at least 'wb_age'. At
...
//    was to a portion of the file which was already uploaded).
...
function _writeback() {
    var now = new Date().getTime();
    var threshold = now - wb_age;

console.log('JJJJ _writeback threshold ' + threshold);

    // When we use 'in', the elements in wb_pending are probably ordered by
    // the order in which they were defined. On startup, when pulling out of
    // the DB, we define them in the order we get them from the DB which, since
    // its a btree, will be alphabetical. Thus, we have to look at each entry
    // to see if its time to write it back.
    //
    // Once we start processing the list, we keep making a pass over the list
    // until we have made a full pass with no writeback. At that time, if there
    // are still elements in the list, we schedule ourselves to wakeup later
    // and process the list again.
    var wrote_back = true;
    ...
    while (wrote_back) {
        ...
        for (fname in wb_pending) {
            if (threshold > wb_pending[fname]) {
console.log('    JJJJ wb file: ' + fname);
                delete wb_pending[fname];

                // XXX JJ here is where we do the writeback

                wrote_back = true;

                if (!wb_pending[p]) {
                    // if no more writes while we were writing back, we
                    // remove it from the DB since its no longer dirty
                     var key = sprintf(DIRTY_KEY_FMT, fname);
//                     self.db.batch()
//                         .del(key)
 ...
 * `path` is the local cache file, not the manta path.
 */
Cache.prototype.dirty = function dirty(p, cb) {
    assert.string(p, 'path');
    assert.func(cb, 'callback');
    ...
    if (!writeback_enabled) {
        ...
        setTimeout(_writeback, self.wbtime + 5);
    }
    self.db.batch()
        .put(k, now)
