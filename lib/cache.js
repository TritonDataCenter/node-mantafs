...
// we have cached locally will be checked to be sure it is not stale.
    ...
    function _doUpload(_arg, _cb) {
        ...
        fs.stat(cachefile, function onStatDone(s_err, stats) {
            ...
            rstream.once('open', function onOpen() {
                self.manta.put(_arg.fname, rstream, m_opts,
                  function (err) {
                    if (err) {
...
Cache.prototype._grow_cache = function _grow_cache(p, size) {
    ...
    function process_entry(val, nextent) {
        ...
        // if in-use, skip it
        if (val._no_evict || val._in_flight) {
            nextent();
            return;
        }
        ...
        fs.stat(val._cacheFile, function (serr, stats) {
            ...
            // if dirty, skip it
            if (wb_pending[nm] || wb_inprogress[nm]) {
                nextent();
                return;
            }
        ...
        if (err) {
            ...
            self._cached_info_stale(p, c_info, function (m_err, stale, m_info) {
                ...
                if (stale) {
                    ...
                    // put it back into the LRU cache
            ...
            self._rm_from_pdir(p, function (r_err) {
                if (r_err) {
                 ...
                 if (info.extension === 'directory') {
                     ...
                     // Assume the directory is empty so size is 0
...
Cache.prototype._invalidate = function _invalidate(p, m_info, cb) {
    ...
    if (m_info.extension === 'directory') {
        ...
        // the cached file. We do know how many entries the directory object
        // has so instead of stat-ing, we estimate that each directory entry
        // consumes 200 bytes. This lets us approximate the amount of space
        // freed in the cache.
        var n_ents = m_info.headers['result-set-size'];
        if (n_ents) {
            size = n_ents * 200;
        } else {
            size = 0;
        ...
        self.db.get(k, function (err, val) {
            ...
            if (entry.type !== 'directory') {
                ...
                var info = {
                    ...
                    _fhandle: fh,
                    _cacheFile: path.join(self.location, 'fscache', fh)
                };
                ...
                // XXX JJ can prepop dirs if we handle
                // headers['result-set-size'] differently. For now dirs just
                // pull in as we get them
                // dirs only have entry.mtime
                _cb();
            }
        });
    }
    ...
    function check_object(now, ttl, key, p, c_info) {
        ...
        fs.stat(c_info._cacheFile, function (err, stats) {
            ...
            if ((now - ftime.getTime()) > ttl) {
                ...
                // The file is fresh, put info from DB into the LRU cache and
                // add to the current cache space used.
                log.trace('not stale, keep: %s', p);
                self._grow_cache(p, stats.size);
                self.cache.set(key, c_info);
                stale_check_cnt--;
                if (db_keys_done && stale_check_cnt === 0)
                    self.emit('ready');
            }
