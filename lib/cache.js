...
function _writeback() {
    ...
    function _doUpload(_arg, _cb) {
        ...
        wb_context.log.debug('_doUpload %s => %s', _arg.cachefile, _arg.fname);
        ...
        rstream.once('open', function onOpen() {
            wb_context.manta.put(_arg.fname, rstream, function (err) {
                if (err) {
                    wb_context.log.error(err,
    ...
    function _wb_done(err) {
        if (err) {
            wb_context.log.error(err, '_wb_done');
            // XXX JJ generalize error handling
            // e.g. if (err.name === 'UploadTimeoutError') ...
            // other manta errors, etc. On upload timeout we won't have called
            // the _fileUploaded function in the pipeline, so we still have
            // the file in the DB, but not the wb_pending list. For now, just
            // wait 60 seconds and retry.
            setTimeout(_writeback, (60000));
        } else {
            // take another lap
            _writeback();
        }
    ...
    for (fname in wb_pending) {
        if (threshold > wb_pending[fname].timestamp) {
            ...
            // here is where we initiate the writeback
            vasync.pipeline({
                funcs: [
                    _doUpload,
                    _fileUploaded
            ...
            // break out of the loop since we'll callback to _writeback when
