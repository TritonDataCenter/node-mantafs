...
// the file can be kept as long as it is not stale. The second option is
// the maximum size of the cache. When pulling an object into the cache, if
...
// a file ages out, we checkin with Manta to be sure our cached copy is not
...
// XXX JJ we also dispose when we rm or rename a file
            wb_context.log.debug('dispose(%s)', k);
        },
...
Cache.prototype.mv = function mv(oldpath, newpath, cb) {
    ...
    function cleanup() {
        self._del_on_rename(oldpath, function (d_err) {
            ...
            self.dirtyParent(newpath, function (p_err) {
                if (p_err) {
...
// Check the cached file to see if it is stale. If it is, invalidate
// the cached copy so that we re-download it from Manta on the next access.
Cache.prototype.refresh = function refresh(p, cb) {
    ...
    self.get(p, function (err, c_info) {
        ...
        // We can't use _stat here since that has a side-effect of saving into
        // the cache, which we don't want. We just want to check the manta info
        // directly to compare with our cached value to see if we're stale.
        self.manta.stat(p, function onStatDone(err2, m_info) {
            if (err2) {
                 log.error(err2, 'refresh manta stat (%s): failed', p);
                 cb(err2);
                 return;
            }
            ...
            if (c_info.extension === 'directory') {
                ...
                // This is not ideal since the dir could have changed but have
                // the same number of entries, but its the best we can do.
...
Cache.prototype.invalidate = function invalidate(p, m_info, cb) {
    assert.string(p, 'path');
    ...
    if (!this.has(p)) {
        // no data for this file is cached, we're done
        log.trace('invalidate(%s): nothing cached, done', p);
        cb(null);
        return;
    }

    this.get(p, function (err, c_info) {
        if (err) {
            log.error(err, 'invalidate(%s): failed', p);
            cb(err);
            return;
        }
        ...
        // we only need to update the info in the cache
        self.db.batch()
            .put(key, m_info)
            .write(function onBatchWrite(err2) {
                if (err2) {
        ...
        self.db.once('ready', function onDatabase() {
            ...
            keys.on('data', function onDbEntry(data) {
                ...
                if (d_re.test(data.key)) {
                    ...
                    var k = data.key.split(':').pop();
                    var v = data.value;
// XXX v.size does not mean that the file is cached, just that we've
// loaded the dir entry
// XXX JJ           self.size += v.size;
