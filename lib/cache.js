...
// Track additional space used when we are caching an object file. If we hit
// the cache size limit, initiate some eviction to get us under the limit.
Cache.prototype._grow_cache = function _grow_cache(p, size) {
    ...
    this.curr_size += size;
    log.trace('_grow_cache(%s, %d): %d', p, size, this.curr_size);
};
