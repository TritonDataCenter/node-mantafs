...
function init(thisp) {
    assert.object(thisp, 'FsCache');
    assert.ok(thisp instanceof FsCache, 'FsCache');
 ...
 * - manta<MantaClient>: node-manta client
...
function FsCache(opts) {
    ...
    // these are stable uuid/name mappings, so no expiry
    this._fhandle_cache = LRU({
        max: 10000
    });
    this._fname_cache = LRU({
        max: 10000
    });
 ...
 *
 */
FsCache.prototype.close = function close(cb) {
 ...
 *
 */
FsCache.prototype.readdir = function readdir(_path, cb) {
    assert.string(_path, 'path');
 ...
 *
 */
FsCache.prototype.stat = function stat(_path, cb, internal) {
    assert.string(_path, 'path');
    assert.func(cb, 'callback');
 ...
 *
 */
FsCache.prototype._fhandle = function _fhandle(_path, cb) {
     assert.string(_path, 'path');
     assert.func(cb, 'callback');
     ...
     if ((fhandle = this._fhandle_cache.get(k))) {
         cb(null, fhandle);
         return;
     }
        ...
        self.db.batch()
            ...
            .write(function (err2) {
                if (err2) {
                    ...
                    self._fhandle_cache.set(k, fhandle);
                    self._fname_cache.set(k2, _path);
                    cb(null, fhandle);
                }
            });
    });
 ...
 *
 */
FsCache.prototype._stat = function _stat(_path, cb) {
    assert.string(_path, 'path');
    assert.func(cb, 'callback');
...
//         fscache.once('ready', function () {
//             fscache.stat('/mark.cavage/stor/tmp', function (err, stats) {
//                 assert.ifError(err);
//                 fscache.stat('/mark.cavage/stor/tmp', function (err, stats) {
//                     assert.ifError(err);
//                     console.log(stats)
