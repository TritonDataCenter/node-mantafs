...
// XXX JJ _arg.stats._cacheFile and _arg.stats._fhandle is redundant with the
// values on _arg.stats._manta
            var _opts = {
                path: _arg.stats._cacheFile
            };
            // estimate the dir size here; num entries * 200
            var size;
            var n_ents;
            if (_arg.stats.headers) {
                n_ents = _arg.stats.headers['result-set-size'];
            } else {
                n_ents = _arg.stats._manta.headers['result-set-size'];
            }
            if (n_ents) {
                size = n_ents * 200;
            } else {
