...
// It is critical that the dev and ino values are always the same for the
// same object but particularly an NFS mount. NFS clients get ther fsid and
// fileid from these values and will consider the mount point stale if they
// vary.
function mantaToStats(_path, info) {
    ...
    stats.dev = hash(path.dirname(_path));
    stats.ino = hash(_path);
 ...
 * synchronously:
 *
 * - if the oldPath is a directory, fail right away
 * - if the oldPath was cached and dirty, upload it
 * - delete the newPath unconditionally
...
MantaFs.prototype.rename = function rename(oldPath, newPath, cb) {
    ...
    function mantaMove() {
        self.unlink(newPath, function (err) {
            // ignore error since its common for the newPath to not exist
    ...
    self.stat(oldPath, function (s_err, o_info) {
        if (s_err) {
            log.trace(s_err, 'rename(%s): failed', oldPath);
            cb(s_err);
            return;
        }

        if (o_info._manta.extension === 'directory') {
            s_err = new ErrnoError('EISDIR', 'rename');
            log.trace(s_err, 'rename(%s): isDirectory', oldPath);
            cb(s_err);
            return;
        }
    ...
    this.stat(p, function (s_err, stats) {
        if (s_err) {
            ...
            self.readdir(p, function (rd_err, files) {
                ...
                self.cache.del(p, function (err) {
                    if (err) {
                        log.trace(err, 'rmdir(%s): failed', p);
                        cb(err);
                    } else {
                        log.trace('rmdir(%s): done', p);
                        cb();
                    }
                });
            });
...
MantaFs.prototype.unlink = function unlink(p, cb) {
    ...
    self.stat(p, function (s_err, info) {
        if (s_err) {
            log.trace(s_err, 'unlink(%s): failed', p);
            cb(s_err);
            return;
        }

        if (info._manta.extension === 'directory') {
            s_err = new ErrnoError('EISDIR', 'unlink');
            log.trace(s_err, 'unlink(%s): isDirectory', p);
            cb(s_err);
            return;
        }
