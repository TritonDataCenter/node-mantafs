...
MantaFs.prototype.fsync = function fsync(fd, cb) {
    ...
    // XXX JJ _fds[].stats never defined
    if (!(this._fds[fd]) || !this._fds[fd].stats) {
        ...
        this.cache.writeback_now(this._fds[fd].stats._path, cb);
    }
};
 ...
 * use this as an indication that the client wants to refresh the cache.
 */
MantaFs.prototype.utimes = function utimes(p, atime, mtime, cb) {
    ...
    this.cache.get(p, function (err, c_info) {
        if (err) {
            log.error(err, 'utimes cache get (%s): failed', p);
            err.syscall = 'stat';
            cb(err);
            return;
        }
        ...
        if (!c_info) {
            log.trace('utimes(%s): not cached, done', p);
            cb(null);
            return;
        }
        ...
        self.manta.stat(p, function onStatDone(err2, m_info) {
            if (err2) {
                 log.error(err2, 'utimes manta stat (%s): failed', p);
                 cb(err2);
                 return;
            }
            ...
            if (stale) {
                self.cache.invalidate(p, cb);
            } else {
                log.trace('utimes(%s): not stale, done', p);
                cb(null);
            }
        });
    });
...
MantaFs.prototype.rename = function rename(oldPath, newPath, cb) {
    ...
    function mantaMove() {
        self.manta.ln(oldPath, newPath, function onLink(err) {
            ...
            self._ensureParent(newPath, function onParent(err2, pstats) {
                if (err2) {
                    cb(err2);
                    return;
                }
...
// will have to go out and hit manta. This uses _stat which always goes to
// manta. This is different from the stat() function which does not return
// the raw manta info.
MantaFs.prototype._get_manta_info = function _get_manta_info(p, cb) {
    ...
    this.cache.get(p, function onCacheGet(err, m_info) {
        ...
        if (!m_info) {
            self._stat(p, function onStatDone(err2, stats) {
                if (err2) {
