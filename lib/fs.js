...
MantaFs.prototype.open = function open(p, flags, mode, cb) {
    ...
    function _checkForDownload(_arg, _cb) {
        ...
        self._stat(p, function (s_err, info) {
            if (s_err) {
    ...
    function _zero(_arg, _cb) {
        ...
        self.manta.zero(p, function (p_err, info) {
            if (p_err) {
                _cb(p_err);
                return;
            }
...
MantaFs.prototype.read = function read(fd, buf, off, len, pos, cb) {
    ...
    this.cache.dataInCache(fmap.path, _pos + len, function (c_err, ready, info) {
        if (c_err) {
...
MantaFs.prototype.truncate = function truncate(p, len, cb) {
    ...
    this.stat(p, function (ms_err, stats) {
        if (ms_err) {
        ...
        self.manta.zero(p, function (m_err, m_info) {
            if (m_err) {
                log.trace(m_err, 'truncate(%s): failed (manta)', p);
                cb(m_err);
                return;
            }
            ...
            self._stat(p, m_info, function (s_err, info) {
                if (s_err) {
                    log.trace(s_err, 'truncate(%s): failed (stat)');
                    cb(s_err);
                    return;
                }

                fs.truncate(info._cacheFile, function (err) {
                    if (err) {
                        log.trace(err, 'truncate(%s): failed (fs)');
                        cb(s_err);
                        return;
                    }

                    // TODO cancel outstanding download
                    log.trace(err, 'truncate(%s): done');
                    cb(null);
                });
            });
        });
...
MantaFs.prototype.unlink = function unlink(p, cb) {
    ...
    this.manta.unlink(p, function (m_err) {
        if (m_err) {
            log.trace(m_err, 'unlink(%s): entered', p);
            cb(m_err);
        } else if (!self.cache.has(p)) {
        ...
        function onReady() {
            ...
            vasync.pipeline({
                ...
                if (err) {
                    ...
                    self.cache.dirty(fmap.path, function (d_err) {
                        if (d_err) {
                            log.warn(d_err, 'dirty(%s): failed', fmap_path);
                            cb(d_err);
                        } else {
