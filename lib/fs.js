...
function MantaFs(opts) {
    ...
    // Fairly silly use, but it does track the tail
    this.dirtyAge = opts.dirtyAge * 1000;
    this.dirty = LRU({
        dispose: this._upload.bind(this),
        max: opts.dirty,
        maxAge: opts.dirtyAge * 1000
    });
...
MantaFs.prototype.close = function close(fd, cb) {
    ...
    var dk = sprintf(DIRTY_KEY_FMT, stats._path);
...
MantaFs.prototype.open = function open(_path, flags, mode, cb) {
    ...
    // For now we create a zero-byte object in manta synchronously to
    // avoid directory conflicts, etc
    function creat() {
    ...
    this.stat(_path, function onStat(s_err, stats) {
        ...
        case 'w':
        case 'w+':
            self.truncate(_path, _assign_fd);
            break;
...
MantaFs.prototype.write = function write(fd, buf, off, len, pos, cb) {
    ...
    this._waitForData(stats, (pos + len), function _write(err) {
        ...
        fs.open(fname, map.flags, function onOpen(o_err, _fd) {
            if (o_err) {
                cb(o_err);
                return;
            }
...
MantaFs.prototype._stage = function _stage(stats, cb) {
    ...
    function timer() {
        clearTimeout(self._timers[stats._path]);
        self._timers[stats._path] = setTimeout(function () {
            self.dirty.del(key);
        }, self.dirtyAge);
    }

    if (!self.dirty.has(key)) {
        this.dirty.set(key, val);
...
MantaFs.prototype._unstage = function _unstage(_path, cb) {
    ...
    if (this._timers[_path]) {
        clearTimeout(this._timers[_path]);
        delete this._timers[_path];
    }
    ...
    this._unstageFromParent(_path, function (err) {
        ...
        if (!self.dirty.has(dk)) {
            log.debug('_unstage(%s): not dirty; done', _path);
            cb();
            return;
        }

        log.debug('_unstage(%s): deleting from dirty list', _path);
        self.db.del(dk, function (err2) {
            if (err2)
                log.debug(err2, '_unstage(%s): delete db.dirty failed', _path);

            self._no_upload[dk] = true;
            self.dirty.del(dk);
 ...
 *
 */
MantaFs.prototype._upload = function _upload(key) {
    assert.string(key, 'key');

    if (this._no_upload[key]) {
        delete this._no_upload[key];
        return;
    }
    ...
    this.db.get(key, function onDBGet(db_err, val) {
        ...
        if (self._timers[val.name]) {
            clearTimeout(self._timers[val.name]);
            delete self._timers[val.name];
        }
