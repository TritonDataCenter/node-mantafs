...
MantaClient.prototype.cacheObject = function cacheObject(p, opts, cb) {
    ...
    this.client.get(p, function onGet(err, mstream) {
        ...
        mstream.on('error', function onMantaGetError(m_err) {
...
MantaClient.prototype.ln = function ln(oldpath, newpath, cb) {
    ...
    log.trace('ln(%s %s): entered', oldpath, newpath);
    this.client.ln(oldpath, newpath, function onLn(err) {
        if (err) {
            log.trace(err, 'ln(%s %s): failed', oldpath, newpath);
            cb(errno(err, 'ln'));
        } else {
            log.trace('ln(%s %s): done', oldpath, newpath);
            cb(null);
        }
    });
};
...
/**
 * Performas a Manta put on an object
 *
 * p -> manta path
...
MantaClient.prototype.put = function put(p, input, opts, cb) {
    ...
    log.trace('put(%s): entered', p);

    this.client.put(p, input, opts, function (err) {
        if (err) {
